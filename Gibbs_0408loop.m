...
%% mkdir('Gibbs0408') %only once CREATE DIRECTORY
mkdir('./Gibbs0408/workspace_Gibbs_BI150_ITE300');
mkdir('./Gibbs0408/matlab_output_Gibbs')
...
min = [1,2,3,4,5,6];
TOT_RUN = 5;
mkdir('./Gibbs0408/error_per_VOC_TOP')

for VOC = VOC_list%VOC=20
    count = count + 1;
    mkdir(['./Gibbs0408/workspace_Gibbs_BI150_ITE300/VOC' int2str(VOC) '/']);
    mkdir(['./Gibbs0408/matlab_output_Gibbs/VOC' int2str(VOC) '/']);
    ...
    for TOP = TOP_list% TOP = 3
        ...
        for k = min%k =1
            ...
            for ii = 1:2%ii =1 
		        ...
		        clear DS WS
		        save(['./Gibbs0408/matlab_output_Gibbs/VOC' int2str(VOC) '/run' int2str(ii) 'matlab_result_DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) 'BURNIN' num2str(BURNIN) 'ITER' num2str(ITER) 'LAG' num2str(LAG) '.mat'], 'Phi','Theta','LL','LLall', 'elapse_time', 'rel_dif_word_gibbs','BURNIN', 'ITER', 'LAG', 'alpha', 'trd_geo_Gibbs', 'beta');
		        save(['./Gibbs0408/workspace_Gibbs_BI150_ITE300/VOC' int2str(VOC) '/run' int2str(ii) 'matlab_result_DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.mat'], '-regexp', '^(?!(out_gcond_gibbs|out_word_gibbs)$).');
	    ...
	    save(['./Gibbs0408/error_per_VOC_TOP/Error_summary_VOC' int2str(VOC) 'TOP' num2str(TOP) 'DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.mat'], 'out_gcond_gibbs' ,'out_word_gibbs' ,'out_time_gibbs');
    end
end
save('./Gibbs0408/after2_5runs.mat');
