...
mkdir('./data_gene_0407/workspace');
mkdir('./data_gene_0407/matlab_output')
mkdir('./data_gene_0407/error_per_VOC_TOP')
%% }
addpath('./TENSORLAB')
...
for VOC = VOC_list %VOC = 10
    %count = count + 1;
    mkdir(['./data_gene_0407/workspace/VOC' int2str(VOC) '/']);
    mkdir(['./data_gene_0407/matlab_output/VOC' int2str(VOC) '/']);    
...
%                 load(['./LDA/test0321/parameter_original_DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) 'index_expe' num2str(ii) '.mat']);
                ...
                save(['./data_gene_0407/workspace/VOC' int2str(VOC) '/run' int2str(ii) 'matlab_result_K' num2str(TOP) 'DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.mat'], '-regexp', '^(?!(out_gcond|out_word|out_alpha)$).');
                %mkdir(['./LDA/test0321/matlab_output/run' int2str(ii) '/']);
                %writetable(PWt,['./data_gene_0407/matlab_output/VOC' int2str(VOC) '/run' int2str(ii) 'Parameter_RC_vs_ORI_DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.txt'],'Delimiter','\t');
                %writetable(PW_p,['./data_gene_0407/matlab_output/VOC' int2str(VOC) '/run' int2str(ii) 'hypara_geocond_summary_value_K' num2mstr(TOP) 'DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.txt'],'Delimiter','\t');
        ...
        save(['./data_gene_0407/error_per_VOC_TOP/Error_summary_VOC' int2str(VOC) 'TOP' num2str(TOP) 'DN' num2str(i) 'NT' num2str(j) 'VOC' num2str(VOC) 'ortho_minus' num2str(k) '.mat'], 'out_gcond' ,'out_alpha' ,'out_word' ,'out_time' ,'out_rel_dif_alha');
    end
end
save('./data_gene_0407/after90runs_times_9voc.mat');
%% Until here--------------------------------------------------------
